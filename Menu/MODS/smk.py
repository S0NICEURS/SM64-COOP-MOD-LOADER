...
# URL de l'image
image_url = "https://i.ibb.co/6F465q9/1711317160135.png"
...
def create_main_window():
    ...
    try:
        ...
        title = tk.Label(root, text="Super Mario Kart", font=("Helvetica", 20, "bold"), fg="white", bg="black")
        title.pack(pady=5)

        # Ajouter le texte "Nom de l'auteur"
        author = tk.Label(root, text="By Skeltan, djoslin0", font=("Helvetica", 16), fg="white", bg="black")
        author.pack(pady=5)
...
def run_minecraftDL():
    try:
        script_path = os.path.join(os.path.dirname(__file__), 'smkDL.py')
        subprocess.run(['python', script_path], check=True)
        print("Le script smkDL.py a été exécuté avec succès.")
    except Exception as e:
        print(f"Erreur lors de l'exécution de smkDL.py : {e}")
