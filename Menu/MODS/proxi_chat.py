...
# URL de l'image
image_url = "https://i.ytimg.com/vi/PUXkrlhqTeI/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLAQgpHPniSyVGWMzA2zetFnu9BmTw"
...
def create_main_window():
    ...
    try:
        ...
        title = tk.Label(root, text="Proximity Chat", font=("Helvetica", 20, "bold"), fg="white", bg="black")
        title.pack(pady=5)

        # Ajouter le texte "Nom de l'auteur"
        author = tk.Label(root, text="By Drenchy", font=("Helvetica", 16), fg="white", bg="black")
        author.pack(pady=5)

        # Ajouter le texte "Description du mod"
        description = tk.Label(root, text="Turn on your microphone and talk to other players", font=("Helvetica", 12), fg="white", bg="black", wraplength=400)
...
def run_minecraftDL():
    try:
        script_path = os.path.join(os.path.dirname(__file__), 'proxi_chatDL.py')
        subprocess.run(['python', script_path], check=True)
        print("Le script proxi_chatDL.py a été exécuté avec succès.")
    except Exception as e:
        print(f"Erreur lors de l'exécution de proxi_chatDL.py : {e}")
