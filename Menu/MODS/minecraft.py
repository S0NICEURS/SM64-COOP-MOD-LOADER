...
# URL de l'image
image_url = "https://i.ibb.co/52wdyGh/227-81aa03bdd52f0a397fe82c5009368f69.png"
...
def create_main_window():
    ...
    try:
        ...
        title = tk.Label(root, text="Minecraft + V1.1", font=("Helvetica", 20, "bold"), fg="white", bg="black")
        title.pack(pady=5)

        # Ajouter le texte "Nom de l'auteur"
        author = tk.Label(root, text="By zKevin, Code D", font=("Helvetica", 16), fg="white", bg="black")
        author.pack(pady=5)

        # Ajouter le texte "Description du mod"
        description = tk.Label(root, text="Push D-pad Left & Right to change blocks, Y to place blocks, X to remove block.", font=("Helvetica", 12), fg="white", bg="black", wraplength=400)
...
def run_minecraftDL():
    try:
        script_path = os.path.join(os.path.dirname(__file__), 'minecraftDL.py')
        subprocess.run(['python', script_path], check=True)
        print("Le script minecraftDL.py a été exécuté avec succès.")
    except Exception as e:
        print(f"Erreur lors de l'exécution de minecraftDL.py : {e}")
