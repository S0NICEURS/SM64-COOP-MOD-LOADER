...
# URL de l'image
image_url = "https://i.ytimg.com/vi/hyuXu4Bzugs/hq720.jpg?sqp=-oaymwEcCNAFEJQDSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLClyZwS_1SB2RBkhCVwzpNlpeatIA"
...
def create_main_window():
    ...
    try:
        ...
        title = tk.Label(root, text="Mario Hunt", font=("Helvetica", 20, "bold"), fg="white", bg="black")
        title.pack(pady=5)

        # Ajouter le texte "Nom de l'auteur"
        author = tk.Label(root, text="By EmilyEmmi", font=("Helvetica", 16), fg="white", bg="black")
        author.pack(pady=5)

        # Ajouter le texte "Description du mod"
        description = tk.Label(root, text="", font=("Helvetica", 12), fg="white", bg="black", wraplength=400)
...
def run_minecraftDL():
    try:
        script_path = os.path.join(os.path.dirname(__file__), 'mario_huntDL.py')
        subprocess.run(['python', script_path], check=True)
        print("Le script mario_huntDL.py a été exécuté avec succès.")
    except Exception as e:
        print(f"Erreur lors de l'exécution de mario_huntDL.py : {e}")
